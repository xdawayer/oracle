...
// OUTPUT: 导出 detail 路由（懒加载 AI 详情解读）。
...
type DetailType = 'elements' | 'aspects' | 'planets' | 'asteroids' | 'rulers';
...
detailRouter.post('/', async (req, res) => {
  try {
    ...
    const validTypes: DetailType[] = ['elements', 'aspects', 'planets', 'asteroids', 'rulers'];
